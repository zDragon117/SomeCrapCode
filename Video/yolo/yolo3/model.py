...
from keras.layers import Add, Concatenate, Conv2D, UpSampling2D, ZeroPadding2D
...
def darknet_conv_2d_bn_leaky(*args, **kwargs):
    no_bias_kwargs = {'use_bias': False}
    no_bias_kwargs.update(kwargs)
    return compose(
        darknet_conv_2d(*args, **no_bias_kwargs),
        BatchNormalization(),
        LeakyReLU(alpha=0.1))
...
def resblock_body(x, num_filters, num_blocks):
    ...
    x = darknet_conv_2d_bn_leaky(num_filters, (3, 3), strides=(2, 2))(x)
    for i in range(num_blocks):
        y = compose(
            darknet_conv_2d_bn_leaky(num_filters // 2, (1, 1)),
            darknet_conv_2d_bn_leaky(num_filters, (3, 3)))(x)
        x = Add()([x, y])
    return x


def darknet_body(x):
    x = darknet_conv_2d_bn_leaky(32, (3, 3))(x)
...
def make_last_layers(x, num_filters, out_filters):
    x = compose(
        darknet_conv_2d_bn_leaky(num_filters, (1, 1)),
        darknet_conv_2d_bn_leaky(num_filters * 2, (3, 3)),
        darknet_conv_2d_bn_leaky(num_filters, (1, 1)),
        darknet_conv_2d_bn_leaky(num_filters * 2, (3, 3)),
        darknet_conv_2d_bn_leaky(num_filters, (1, 1)))(x)
    y = compose(
        darknet_conv_2d_bn_leaky(num_filters * 2, (3, 3)),
        darknet_conv_2d(out_filters, (1, 1)))(x)
    return x, y
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x = compose(
        darknet_conv_2d_bn_leaky(256, (1, 1)),
        UpSampling2D(2))(x)
    x = Concatenate()([x, darknet.layers[152].output])
    x, y2 = make_last_layers(x, 256, num_anchors * (num_classes + 5))

    x = compose(
        darknet_conv_2d_bn_leaky(128, (1, 1)),
        UpSampling2D(2))(x)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5):
    ...
    for l in range(3):
        ...
        def loop_body(b, b_ignore_mask):
            true_box = tf.boolean_mask(y_true[l][b, ..., 0:4], object_mask_bool[b, ..., 0])
            iou = box_iou(pred_box[b], true_box)
            best_iou = K.max(iou, axis=-1)
            b_ignore_mask = b_ignore_mask.write(b, K.cast(best_iou < ignore_thresh, K.dtype(true_box)))
            return b + 1, b_ignore_mask
